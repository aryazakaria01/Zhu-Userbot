    ...
    if event.reply_to_msg_id and len(args) != 2:
        ...
        try:
            ...
            return await event.edit(
                "`Terjadi Kesalahan... Mohon Lapor Ke Grup` @kenzusupport", str(err)
            )
...
async def gben(userbot):
    ...
    await dark.edit('`Global Banned Akan Segera Aktif Tuan!!!`')
    ...
    if user:
        if user.id in DEVS:
            return await dark.edit(
                '`Anda Tidak Bisa Melakukan Global Banned, Karena dia pembuatku 🤪`'
            )
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await dark.edit('Global Banned Diaktifkan')
            except BaseException:
                b += 1
    else:
...
async def gunben(userbot):
    ...
    me = await userbot.client.get_me()
    await dark.edit(
        'Memulai Membatalkan Perintah Global Banned, Jangan Jadi Jamet Lagi Ya!!!'
    )
